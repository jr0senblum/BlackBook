...
## Unit 1: Database Schema + ORM + Schemas — `llm_context_mode`
...
- [ ] Create Alembic migration: `alembic revision --autogenerate -m "add_llm_context_mode_to_companies"`
  - Add column `llm_context_mode` (text, NOT NULL, server_default `'accepted_facts'`)
  - Verify migration runs cleanly on `blackbook` DB
- [ ] Update `backend/app/models/base.py` — `Company` model:
...
- [ ] Update `backend/app/schemas/company.py`:
...
- [ ] Update `backend/app/services/company_service.py`:
...
- [ ] Update `backend/app/repositories/company_repository.py`:
...
- [ ] Write tests in `backend/tests/test_repositories/test_unit3_repositories.py` (extend existing):
...
- [ ] Write tests in `backend/tests/test_api/test_companies.py` (extend existing):
...
## Unit 2: InferenceService — `RAW_SYSTEM_PROMPT` + `extract_facts_raw()`
...
- [ ] Add `RAW_SYSTEM_PROMPT` constant in `backend/app/services/inference_service.py`:
...
- [ ] Add `_build_raw_system_prompt(company_context: str | None) -> str` private function:
...
- [ ] Add `async def extract_facts_raw(self, raw_text: str, company_context: str | None = None) -> list[LLMInferredFact]`:
...
- [ ] Refactor `_call_llm(self, user_message)` → `_call_llm(self, user_message, system_prompt=SYSTEM_PROMPT)`:
...
- [ ] Update `InferenceServiceProtocol` in `backend/app/services/ingestion_service.py`:
  - Add `async def extract_facts_raw(self, raw_text: str, company_context: str | None = None) -> list[Any]: ...`
- [ ] Write tests in `backend/tests/test_services/test_inference_service.py` (extend existing):
...
## Unit 3: Context Assembly in IngestionService — `_build_company_context()`
...
- [ ] Add `InferredFactRepository.list_accepted_by_company(company_id, *, limit) -> list[InferredFact]`:
...
- [ ] Add `SourceRepository.list_processed_content(company_id, *, limit) -> list[tuple[str, str | None]]`:
...
- [ ] Add `_build_company_context(self, company_id: UUID) -> str | None` to `IngestionService`:
...
- [ ] Write tests in `backend/tests/test_services/test_ingestion_service.py` (extend existing):
...
## Unit 4: Extraction Mode Detection + Hybrid Dispatch in IngestionService
...
- [ ] Define mode detection logic in `IngestionService`:
...
- [ ] Update `backend/app/services/prefix_parser_service.py`:
...
- [ ] Update `process_source()` in `backend/app/services/ingestion_service.py`:
...
- [ ] Handle errors: wrap both LLM calls in the existing try/except for `InferenceValidationError` and `InferenceApiError`. In hybrid mode, if either pass fails, the entire source is marked `failed` — no partial persistence. This follows §9.5's failure path: "On any validation failure: the Source status is set to failed." The error message should indicate which pass failed (e.g., "Raw extraction failed: LLM returned invalid JSON")
- [ ] Write tests in `backend/tests/test_services/test_ingestion_service.py` (extend existing):
...
- [ ] Write tests in `backend/tests/test_services/test_prefix_parser_service.py` (extend existing):
...
## Unit 5: Source Line Attribution for Raw Mode
...
- [ ] Update `ReviewService.save_facts()` signature and logic:
...
- [ ] Add `_match_raw_source_line(fact, raw_lines: list[str]) -> str | None` to `ReviewService`:
...
- [ ] Update `ReviewServiceProtocol` in `backend/app/services/ingestion_service.py`:
  - Add `raw_lines: list[str] | None = None` parameter to `save_facts` protocol
- [ ] Update `IngestionService.process_source()` callers:
...
- [ ] Write tests in `backend/tests/test_services/test_review_service.py` (extend existing):
...
## Unit 6: Frontend — `llm_context_mode` Selector
...
- [ ] Update `frontend/src/pages/CompanyProfilePage.tsx`:
...
- [ ] Verify build: `npm run build` succeeds with no TypeScript errors
- [ ] Manual smoke test: edit a company's context mode, verify it persists after reload
...
- [ ] `llm_context_mode` column exists on `companies` table with correct default
- [ ] `InferenceService.extract_facts_raw()` extracts facts from raw text with same validation/retry as tagged mode
- [ ] `RAW_SYSTEM_PROMPT` instructs LLM for unstructured text extraction with company context
- [ ] `IngestionService.process_source()` correctly detects tagged/raw/hybrid mode and dispatches accordingly
- [ ] Company context assembly respects `llm_context_mode` and character budget
- [ ] Source line attribution works for raw-mode facts (matching against raw text lines)
- [ ] Hybrid mode: tagged pass + raw pass combined; if either pass fails, source marked failed (fail-all per §9.5)
- [ ] Frontend: company edit form includes `llm_context_mode` selector
- [ ] All existing tests pass (backward compatible)
- [ ] All new tests pass
- [ ] Live test: upload unannotated text file → facts extracted → appear in pending review queue
- [ ] Live test: upload mixed file (some tagged, some plain text) → hybrid extraction produces combined facts
