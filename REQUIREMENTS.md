...
- Entity disambiguation: during review, for each inferred entity, present a ranked list of existing same-category entities already associated with the company, ordered by similarity to the inferred value (fuzzy character-level matching, not alphabetical); allow the investigator to accept as new, merge with an existing entity, or correct manually; applies to entity categories that have corresponding entity tables (people, functional areas) — see §10.4 for per-category behavior. **Note**: categories without entity tables (technology, process, product, cgkra-\*, swot-\*, other) return empty candidate arrays per §10.4. The matching algorithm uses `SequenceMatcher` (character-level); it handles partial matches (e.g., "Eng" surfaces "Engineering") but does not resolve abbreviations or synonyms (e.g., "k8s" scores low against "Kubernetes" — ≈ 0.31).
