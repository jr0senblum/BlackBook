...
- [x] Extend `InferredFactRepository` in `backend/app/repositories/inferred_fact_repository.py`:
  - `list_linked_to_person(company_id: UUID, person_id: UUID, person_name: str, primary_area_id: UUID | None) -> list[InferredFact]` — return accepted/corrected facts linked to this person. `person_name` is required for the name-matching query (set 1 below). Query strategy: **# DECISION** (see rationale below): this method returns the union of two sets, deduplicated by fact ID, ordered by `created_at`:
    1. Facts with `category = 'person'` where `inferred_value` ILIKE `%person_name%` (case-insensitive contains match against the person's name) — these are the facts that created or identified this person. **This extends beyond §10.5** which specifies only area-linked facts; the extension is deliberate to show the investigator which facts sourced this person.
...
- [x] **Complete `PersonService.get_person()`** in `backend/app/services/person_service.py` — enrich the currently minimal response:
  ...
  - Fetch linked facts: call `self._inferred_fact_repo.list_linked_to_person(company_id, person.id, person.name, person.primary_area_id)` — returns `list[InferredFact]`.
...
- [x] Add Pydantic schemas to `backend/app/schemas/person.py` (new file):
  - `PersonCreateInput`:
    ...
    - `title: str | None = None`
    - `primary_area_id: UUID | None = None` — UUID for Pydantic format validation; consistency with all other entity ID fields. If provided and the area does not exist for this company, `PersonService.create_person()` catches the `IntegrityError` and re-raises it as `InvalidFKError` (422), which the global `DomainError` handler in `main.py` serializes as `{"error": {"code": "invalid_fk", ...}}`.
...
- [x] Create route handlers in `backend/app/api/v1/people.py` (new file):
  - `GET /companies/{company_id}/people` — call `person_service.list_people()`; inject `FunctionalAreaService` as a second DI dependency and call `functional_area_service.list_areas(company_id)` to build an area-name map; return `PersonListResponse`. **Note**: area names are fetched via the public `FunctionalAreaService` API — the route handler must NOT access `person_service._functional_area_repo` (private attribute / layer violation).
  - `POST /companies/{company_id}/people` — parse `PersonCreateInput`; call `person_service.create_person()`; return `PersonCreatedResponse` with status 201. FK errors are raised as `InvalidFKError` by the service and serialized by the global handler (no `HTTPException` in the route).
...
- [x] Write tests in `backend/tests/test_api/test_people.py` (new file):
  ...
  - `test_create_person_invalid_area_id` — POST with non-existent `primary_area_id` -> 422 with `{"error": {"code": "invalid_fk", ...}}` top-level envelope (not `{"detail": ...}`)
  ...
  - `test_update_person_invalid_area_id` — PUT with non-existent `primary_area_id` -> 422 with `{"error": {"code": "invalid_fk", ...}}` envelope
...
**571 tests passing after post-review fixes (D1, D2, A1, S1, S2, S3).**
