...
## Unit 2: InferenceService — LLM Prompt + Validation + Retry
...
## Unit 3: Source + InferredFact Repositories
...
## Unit 4: IngestionService + Company Routing + Background Worker
...
## Unit 5: ReviewService — Accept + Dismiss
...
## Unit 6: Pending Review API Endpoints
...
- [ ] Write API tests in `backend/tests/test_api/test_pending.py`
  - `GET /companies/{id}/pending` (default status=pending) returns pending facts
  ...
  - `GET /companies/{id}/pending?status=accepted&category=person` returns accepted person facts
  - `POST .../accept` on a person fact → 200, person created, `entity_id` is the new person's UUID
  - `POST .../accept` on an action-item fact → 200, action_item created, `entity_id` is the new action item's UUID
  - `POST .../accept` on a technology fact → 200, `entity_id` is null
  - `POST .../dismiss` → 200, fact dismissed
  - `POST .../accept` on non-pending fact → 409
  - `POST .../accept` with wrong company_id → 404
...
## Unit 7: Frontend — File Upload + Source List + Pending Review Queue
...
## Unit 8: Integration Verification + Test Suite Completion
...
- [ ] PrefixParserService correctly normalizes all canonical keys, aliases, routing fields, and metadata
- [ ] InferenceService constructs correct LLM prompts, validates responses, and applies retry policy
- [ ] Company routing (nc/c/cid) works correctly with proper error messages on failure
- [ ] File upload stores the file and creates a Source record
- [ ] Background worker processes sources asynchronously (LLM extraction via InferenceService)
- [ ] Failed sources are surfaced with error messages; retry re-enters the queue
- [ ] Pending review queue lists all pending InferredFacts for a company
- [ ] Accept flow works correctly for ALL categories: person (name+title parse), functional-area (create row), action-item (promote to action_items), relationship (name resolution + stub creation), all others (mark accepted)
- [ ] Dismiss flow marks InferredFacts as dismissed
- [ ] Company profile page shows: file upload, source list with status, pending review queue, accepted people/technologies/processes
- [ ] All pytest tests pass (Phase 1 + Phase 2), repeatable across runs
